...
ngx_http_handler(ngx_http_request_t *r)
{
    ...
    if (!r->internal) {
        ...
        r->phase_handler = 0;   //11个阶段中的序号, 0为第一个阶段

    } else {
        cmcf = ngx_http_get_module_main_conf(r, ngx_http_core_module);
        r->phase_handler = cmcf->phase_engine.server_rewrite_index; //11个阶段中的序号
    }
    ...
    r->write_event_handler = ngx_http_core_run_phases;  //由连接的读写事件回调方法调用
    ...
    ngx_http_phase_handler_t *ph)
{
    ...
    if (r->content_handler) {
        //如果设置了content_handler, 则以content_handler优先
        //模块可以在解析配置阶段(例如location配置项)设置content_handler, 从而介入本阶段
